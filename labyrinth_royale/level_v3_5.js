        ...
        for(i = 0; i < players.length; i++){
            for(j = 1; j < layer.length; j++){
                if(j != 2 && j < 6){
                    ...
                    if(layer[j].visible){
                        var tile = map.getTileWorldXY( players[0].sprite.x, players[0].sprite.y, 32, 32, layer[j]);

                        if(tile != null && first_visible) { players[0].lifePoints = 0; }
                   ...
                   for(var i = 0; i < players.length; i++){
                    ...
                    for(i = 1; i < 6; i++){
                        if(i != 2){
                            ...
                            //game.physics.arcade.collide(b, layer[i], function(b, layer){b.kill(); }); 
                        }
                    }
				});
                }
...
function Interface(){
    ...
    this.updateInterface = function(player, orbe){ 
        ...
        if(players[0].changedShield){
            ...
            if (players[0].shield > 2){ shield[2].visible = true; console.log('3'); }
